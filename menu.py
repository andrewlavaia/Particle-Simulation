...
class MainMenu:
    ...
    def drawMenu(self):
        ...
        self.input_n = InputBox(self.window, Point(80.0, 350.0),
                                'unsigned_int', '# of particles: ', 4, 40)
        self.input_color = InputBox(self.window, self.input_n.getPointWithOffset(),
                                    'color', 'color: ', 10, 'random')
        self.input_r = InputBox(self.window, self.input_color.getPointWithOffset(),
                                'unsigned_float', 'radius: ', 4,  5.0)
        self.input_m = InputBox(self.window, self.input_r.getPointWithOffset(),
                                'unsigned_float', 'mass: ', 4,  1.0)
        self.input_shape = InputBox(self.window, self.input_m.getPointWithOffset(),
                                    'shape', 'shape: ', 6,  'Circle')
        self.add_group_btn = Button(self.window, Point(self.input_shape.point.x + 30,
                                    self.input_shape.point.y + 60), 150, 30, 'Add Group')
...
class ParticleTable(ConfigTableBase):
    ...
    def loadRowsFromConfig(self):
        ...
        for name, data in self.data_dict.items():
            ...
            self.table.addRow(key, data['n'], data['color'], data['radius'],
                              data['mass'], data['shape'])
